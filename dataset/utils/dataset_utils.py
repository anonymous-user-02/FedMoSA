...
def check_alt(config_path, train_path, val_path, test_path, num_clients, num_classes):
    # If the config file exists, delete it and all data directories
    if os.path.exists(config_path):
        print(f"Deleting existing dataset configuration at {config_path}...")
        os.remove(config_path)

    # Delete any existing directories for train, validation, and test sets
...
def separate_data(data, num_clients, num_classes, niid=False, balance=False, partition=None, class_per_client=2):
    ...
    if partition == 'pat':
        ...
        for i in range(num_classes):
            ...
            proportions = np.random.dirichlet(np.repeat(alpha, len(selected_clients)))
...
def separate_data_alt(data, num_clients, num_classes, dataset):
    X = [[] for _ in range(num_clients)]
    y = [[] for _ in range(num_clients)]
    statistic = [[] for _ in range(num_clients)]
...
def split_data_alt(X, y, train_size=0.8, val_size=0.1):
...
def save_file_alt(config_path, train_path, val_path, test_path, train_data, val_data, test_data, num_clients,
